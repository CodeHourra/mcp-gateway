...
Agent 接入页增加默认收起的“网关工具与调用调试”。实际调用当前地址的 MCP `tools/list`，不会把所有上游工具冒充为渐进入口目录。受控渐进入口返回 `retrieve_tools`、`describe_tool`、`call_tool_read`、`call_tool_write`、`call_tool_destructive`、`read_cache` 六项；普通聚合模式按该入口真实目录展示。

查看参数表/完整 schema，输入 JSON 对象后执行调用，结果保留完整 MCP 内容及 `isError`。调试使用自动生成的本地客户端 token 和独立连接，复用握手代码，不借用已接入 Agent 的会话。刷新工具保留连接，“重置调试连接”重新握手；关闭网关时清理连接。入口变化后拒绝旧目录请求，调用失败不会自动重试。写入/删除类工具可能影响上游数据，需核对参数后手动执行。
