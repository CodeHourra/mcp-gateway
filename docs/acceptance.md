...
> 0.1.8：服务默认折叠、工具独立层级、配置单独入口；Agent 按配置状态分区展示。三个 agent 并行实现/独立复查，最终原生结果见 [报告](../tests/acceptance/hierarchy-ui-0.1.8-2026-09-07.json)。

> 0.1.7：文件控件全页审计、默认工具分组与按需详情、完整 MCP JSON 编辑、Agent 配置状态和解除。最终隔离原生结果见 [报告](../tests/acceptance/management-ui-0.1.7-2026-09-07.json)。三位协作 agent 分工实现/审查，主 agent 完成集成与原生复验。
...
**当前结论：本轮 0.1.8 已完成构建和隔离原生管理流程检查；当前产物及 DMG 校验见最新构建清单。0.1.2 的复制后生产应用运行报告保留历史版本边界，完整实机验收继续待执行。** 用户已将依赖本人操作的安装、真实服务/客户端和系统检查调整为交付后由我们指导完成，见[安装与实机验证](install-and-verify.md)。本地受控协议和主要 WebView 操作已通过。 已完成服务 CRUD、导入冲突与原始字节恢复、静态认证与多环境变量、受控 OAuth 原生登录/取消/刷新/清除及重启恢复、工具门禁、大结果显示、诊断真实文件保存与脱敏、端口恢复、日志设置生效与主题检查。最后的 OAuth 状态透传、缓存提示和诊断原生文件保存修复已分别在对应测试 bundle 复验。真实第三方服务、五个真实客户端和 OS 菜单/系统行为仍有明确待验收项，见文末。
...
本轮已由三位协作 agent 分别检查控件、实现 Agent 状态、实现完整 JSON 后端，并交叉审查安全保存路径。前端全部输入已扫描：唯一裸文件选择器已修复，其余无第二处 file/select 或 alert/confirm/prompt 默认控件；保留原生 input 的键盘可达性和焦点样式。

- 前端类型/Vite、5 项前端检查、完整 `go test ./...` 通过。新增覆盖 JSON 修改后预览失效、移除确认、失败留稿和成功提示，Agent 状态/精确备份恢复/冲突拒绝/解除，完整配置凭证往返、字段校验、并发变更与失败路径。禁用服务状态不再被核心 idle 覆盖。
- [实际 WKWebView](../tests/acceptance/management-ui-0.1.7-2026-09-07.json) 使用最终 UI、业务代码与打包核心：两组服务默认展示工具且没有自动打开详情；点击服务聚焦详情；JSON 无效内容不丢、跨页面留稿、移除按钮确认门禁、保存后磁盘服务列表及原生环境引用保留；网关监听设置不变。
- 临时五类 Agent 配置分别呈现未配置、已配置接入、客户端已禁用、需更新、同名冲突；从 UI 预览并解除 Cursor 条目后，无关服务与自定义字段保持。状态仅表示配置，不推测在线。
- 原生窗口设为 760×560，实际 WebView 宽 761、文档可用宽 746，scrollWidth 同为 746，无横向溢出。文件 input 可程序聚焦，CSS 裁切原生英文控件，合成 File 选择后显示中文文件名；深色主题实际 computed color-scheme 为 dark。OS 文件对话框、真实键盘焦点环与像素外观仍由用户安装后验证。
- [核心真实 API 检查](../tests/acceptance/full-mcp-config-2026-09-07.json) 证明 validate、添加禁用服务、空数组移除全部服务及非 MCP 设置保留；34 个原生顶层服务字段与锁定核心源码一致。未知字段拒绝，原生 JSON 使用 `${env:NAME}` / `${keyring:NAME}` 引用。

测试使用独立 app 名称/单实例标识/窗口标题和临时 admin 环境值的 Go overlay；其他业务源码及前端与生产一致。CI 模式禁用核心钥匙串，未写真实配置或凭证，未测试真实客户端握手/提供方。测试 app/core 正常退出，受管子进程均结束。首次 probe 因临时 core 数据目录缺失而失败，补齐 fixture 后通过；不是以修改产品代码绕过失败。全构建中一次旧有 1 秒启动清理测试未读到 PID，定向复核和后续全构建均通过。

## 0.1.8 折叠层级和接入分区复验

- 三个 agent 分别负责服务列表重设计、Agent 分区与独立审查；前端类型/Vite、7 项前端回归、完整 Go 测试、diff 检查通过。原有接入识别规则未放宽，新增前端检查覆盖互斥分组、刷新变化、折叠与配置选择互不影响、焦点返回。
- [最终 WKWebView 受控验收](../tests/acceptance/hierarchy-ui-0.1.8-2026-09-07.json)：两个服务初始 `aria-expanded=false`，可见工具区 0，详情未自动打开；展开后有专属标题与分组样式且不打开 inspector；折叠后再次展开保留参数。配置按钮默认进入连接与认证并聚焦。
