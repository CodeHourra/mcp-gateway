...
function stateLabel(state: string) {
  return ({ ready: '可用', connected: '已连接', connecting: '连接中', disconnected: '未连接', disabled: '已禁用', error: '连接失败', auth_required: '需要登录', starting: '启动中', loading: '等待连接状态', running: '运行中', stopped: '已停止', paused: '已暂停', success: '成功', failed: '失败', pending: '进行中', unavailable: '不可用', fresh: '已更新', live: '实时目录', cached: '缓存目录', config_present: '检测到配置', not_configured: '尚未配置' } as Record<string, string>)[state] ?? state
}
...
<template>
  <div class="app-shell">
    ...
    <div class="main-shell">
      ...
      <footer class="app-footer"><span>{{ connected ? `${services.filter(server => ['ready', 'connected'].includes(server.status)).length} / ${services.length} 个服务可用` : snapshot?.servicesSource === 'config' ? `${services.length} 个服务 · 状态待更新` : '等待连接' }}</span><span v-if="snapshot && snapshot.servicesSource !== 'config'">{{ allTools.length }} tools<span v-if="snapshot.gateway.activeCalls"> · {{ snapshot.gateway.activeCalls }} 个调用执行中</span></span><span class="footer-right">本机 · macOS</span></footer>
    </div>
