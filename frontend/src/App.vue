...
const connected = computed(() => !!snapshot.value && !connectionError.value)
...
watch(page, () => {
  ...
  if (page.value === 'agents' && !agentsLoaded.value) void loadAgents()
  if (page.value === 'import') void scanImportSources()
})
watch(selectedId, () => { toolSearch.value = ''; tab.value = 'tools' })
watch(serviceSearch, () => { if (!filteredServices.value.some(server => server.id === selectedId.value)) selectedId.value = filteredServices.value[0]?.id ?? '' })
...
function stateLabel(state: string) {
  return ({ ready: '可用', connected: '已连接', connecting: '连接中', disconnected: '未连接', disabled: '已禁用', error: '连接失败', auth_required: '需要登录', running: '运行中', stopped: '已停止', paused: '已暂停', success: '成功', failed: '失败', pending: '进行中', unavailable: '不可用', fresh: '已更新', live: '实时目录', cached: '缓存目录', config_present: '检测到配置', not_configured: '尚未配置' } as Record<string, string>)[state] ?? state
}
function stateClass(state: string) { return ['ready', 'connected', 'running', 'success'].includes(state) ? 'good' : ['error', 'failed'].includes(state) ? 'bad' : ['auth_required', 'paused', 'pending', 'connecting'].includes(state) ? 'warn' : 'neutral' }
...
async function refresh(silent = false) {
  ...
  try {
    ...
    if (!selected.value) selectedId.value = value.services[0]?.id ?? ''
...
async function loadAgents() { await run(async () => { agents.value = await request<Agent[]>('listAgentAdapters'); agentsLoaded.value = true }) }
async function previewAgent(agent: Agent) { await run(async () => { agentPreview.value = await request<AgentPreview>('previewAgentConfig', { agentId: agent.id }) }) }
async function applyAgent() {
  await run(async () => { if (!agentPreview.value) return; await request('applyAgentConfig', { previewId: agentPreview.value.id }); agentPreview.value = undefined; agents.value = await request<Agent[]>('listAgentAdapters') }, '配置已写入，原文件备份已保留；客户端实际连接以握手状态为准。')
}
...
onMounted(() => { window.addEventListener('gateway:navigate', nativeNavigate); void refresh(); poll = setInterval(() => { if (!busy.value && window.gateway) void refresh(true) }, 5000) })
onUnmounted(() => { window.removeEventListener('gateway:navigate', nativeNavigate); if (poll) clearInterval(poll); refreshGeneration++ })
</script>
...
<template>
  <div class="app-shell">
    <nav class="sidebar" aria-label="应用导航">
      ...
      <div class="local-note"><span class="status-dot" :class="connected ? stateClass(snapshot!.gateway.status) : 'neutral'" />{{ connected ? '仅在本机运行' : '管理接口未连接' }}</div>
    </nav>
    <div class="main-shell">
      ...
      <main>
        <section v-if="page === 'services'" class="page services-page">
          ...
          <div v-else class="master-detail"><div class="service-list"><div class="list-caption">{{ filteredServices.length }} 个服务</div><p v-if="!filteredServices.length" class="empty-state compact">没有匹配的服务</p><button v-for="server in filteredServices" :key="server.id" class="service-row" :aria-pressed="selectedId === server.id" @click="selectedId = server.id"><span class="service-symbol" aria-hidden="true">{{ server.name.slice(0, 1).toUpperCase() }}</span><span class="grow"><strong>{{ server.name }}</strong><small>{{ server.transport === 'http' ? 'HTTP' : server.transport }} · {{ server.toolCount }} tools</small><span class="state-line" :class="stateClass(server.status)"><span class="status-dot" />{{ stateLabel(server.status) }}</span></span></button></div>
            <aside v-if="selected" class="inspector" aria-label="选中服务详情"><div class="inspect-header"><div class="inspect-top"><div class="inspect-name"><span class="service-symbol large" aria-hidden="true">{{ selected.name.slice(0, 1).toUpperCase() }}</span><div><h2>{{ selected.name }}</h2><span class="badge" :class="stateClass(selected.status)">{{ stateLabel(selected.status) }}</span></div></div><label class="check-row"><input type="checkbox" :checked="selected.enabled" :disabled="busy || !connected" @change="mutate('setServiceEnabled', { serviceId: selected.id, enabled: ($event.target as HTMLInputElement).checked }, '服务启用状态已更新。')">启用</label></div><div class="meta-line"><code>{{ endpoint(selected) }}</code><span>{{ selected.toolCount }} tools</span></div><p v-if="selected.statusMessage" class="note" :class="{ 'error-text': selected.status === 'error' }">{{ selected.statusMessage }}</p></div>
              <div class="tab-bar" role="tablist" aria-label="服务详情"><button v-for="item in [{ id: 'tools', label: 'Tools' }, { id: 'connection', label: '连接与认证' }, { id: 'activity', label: '活动' }] as const" :key="item.id" role="tab" :aria-selected="tab === item.id" @click="tab = item.id">{{ item.label }}</button></div>
              <div class="panel-body" role="tabpanel">
                <template v-if="tab === 'tools'"><p v-if="selected.catalogStatus === 'cached' || selected.catalogStatus === 'unavailable'" class="callout">工具目录：{{ stateLabel(selected.catalogStatus) }}。连接恢复后可重新读取。</p><label class="search-box"><span aria-hidden="true">⌕</span><input v-model="toolSearch" aria-label="搜索此服务的工具" placeholder="搜索工具名称或说明"></label><ToolsList :tools="serviceTools" :busy="busy || !connected" @toggle="toggleTool" /></template>
                <template v-else-if="tab === 'connection'"><dl class="detail-list"><div><dt>传输方式</dt><dd>{{ selected.transport === 'http' ? 'Streamable HTTP' : selected.transport }}</dd></div><div><dt>认证方式</dt><dd>{{ { none: '无认证', bearer: 'Bearer Token', api_key: 'API Key', headers: '自定义 Header', env: '环境变量', oauth: 'OAuth' }[selected.auth.type] }}</dd></div><div><dt>认证状态</dt><dd>{{ authStateLabel(selected.authStatus) }}</dd></div><div v-if="selected.sources?.length"><dt>配置来源</dt><dd>{{ selected.sources.join('、') }}</dd></div><div v-if="selected.lastConnected"><dt>最近连接</dt><dd>{{ date(selected.lastConnected) }}</dd></div></dl><div class="actions"><button class="control" :disabled="busy || !connected" @click="mutate('testService', { serviceId: selected.id }, '连接测试已执行，查看服务状态和活动以确认结果。')">测试连接</button><button class="control" :disabled="busy || !connected" @click="mutate('reconnectService', { serviceId: selected.id }, '已请求重新连接。')">重新连接</button><button class="control" @click="beginEdit(selected)">编辑配置</button></div>
                  ...
                  <section class="form-section"><h3>移除服务</h3><p class="muted">移除网关中的配置，保留已生成的配置备份。</p><button class="control danger" :disabled="busy || !connected" @click="askConfirmation('移除服务', `移除“${selected.name}”后，agent 将无法通过网关调用它的工具。`, '移除服务', () => mutate('deleteService', { serviceId: selected!.id }, '服务已移除。'))">移除服务</button></section>
                </template>
        ...
        <section v-else-if="page === 'import'" class="page"><div class="page-head"><div><h1>导入配置</h1><p>收拢已有 MCP，先检查，再合并。</p></div></div><ol class="stepper" aria-label="导入进度"><li v-for="(label, index) in ['选择来源', '检查与合并', '导入完成']" :key="label" :aria-current="importStep === index + 1 ? 'step' : undefined"><span>{{ index + 1 }}</span>{{ label }}</li></ol>
          <div v-if="importStep === 1" class="editor-wrap">
            ...
            <template v-else>
              ...
              <label v-if="importMode === 'file'" class="file-drop"><strong>{{ importFilename || '选择要导入的配置文件' }}</strong><span>文件在本机处理，预览不回显凭证。</span><input type="file" accept=".json,.toml,.jsonc" :disabled="busy" @change="readFile"></label>
        ...
        <section v-else-if="page === 'agents'" class="page"><div class="page-head"><div><h1>Agent 接入</h1><p>让每个 agent 通过一个本机网关使用工具。</p></div><button class="control" :disabled="busy" @click="loadAgents">刷新</button></div><div class="endpoint-card"><div><span class="muted">网关入口</span><code>{{ snapshot?.gateway.address || '等待网关启动' }}</code></div><button class="control" :disabled="!snapshot?.gateway.address" @click="copyAddress">复制地址</button></div><p class="note">配置检查与实际握手分开显示，写入前提供差异预览与备份。</p><div v-if="!agents.length" class="empty-state compact">{{ busy ? '正在读取客户端…' : '尚未读取到客户端适配信息。' }}</div><div v-for="agent in agents" :key="agent.id" class="wide-row"><span class="service-symbol" aria-hidden="true">{{ agent.name.slice(0, 1) }}</span><div class="grow"><strong>{{ agent.name }}</strong><small>{{ agent.message || agent.configPath || '未提供配置位置' }}</small></div><span class="badge neutral">{{ stateLabel(agent.status) }}</span><button class="control" :disabled="busy || !connected" @click="previewAgent(agent)">查看接入配置</button></div><section v-if="agentPreview" class="preview-panel"><div class="page-head"><div><h2>配置变更预览</h2><p class="address">{{ agentPreview.path }}</p></div><button class="control" @click="agentPreview = undefined">关闭</button></div><p v-for="warning in agentPreview.warnings" :key="warning" class="warning-text">{{ warning }}</p><div class="diff-grid"><div><h3>当前配置</h3><pre>{{ agentPreview.before || '文件尚不存在' }}</pre></div><div><h3>应用后</h3><pre>{{ agentPreview.after }}</pre></div></div><div class="actions form-actions"><button class="control primary" :disabled="busy" @click="applyAgent">备份并应用配置</button></div></section></section>
      ...
      <footer class="app-footer"><span>{{ connected ? `${services.filter(server => ['ready', 'connected'].includes(server.status)).length} / ${services.length} 个服务可用` : '等待连接' }}</span><span v-if="snapshot">{{ allTools.length }} tools<span v-if="snapshot.gateway.activeCalls"> · {{ snapshot.gateway.activeCalls }} 个调用执行中</span></span><span class="footer-right">本机 · macOS</span></footer>
    </div>
