...
const props = defineProps<{ tools: Tool[]; busy?: boolean }>()
...
<template>
  ...
  <div v-else class="tools-list">
    <details v-for="tool in tools" :key="tool.id" class="tool-entry">
      <summary class="tool-summary"><span class="chevron" aria-hidden="true">›</span><span class="grow"><strong>{{ tool.name }}</strong><small>{{ tool.description || '上游未提供说明' }}</small><small>{{ tool.serviceName }}<template v-if="tool.catalogStatus === 'cached' || tool.catalogStatus === 'unavailable'"> · {{ tool.catalogStatus === 'cached' ? '缓存目录' : '目录不可用' }}</template></small></span><span class="badge" :class="tool.enabled ? 'good' : 'neutral'">{{ tool.enabled ? '允许调用' : '已禁用' }}</span></summary>
